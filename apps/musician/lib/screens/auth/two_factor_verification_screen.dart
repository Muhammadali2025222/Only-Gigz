...
class _TwoFactorVerificationScreenState extends State<TwoFactorVerificationScreen> {
  ...
  bool _useSms = true; // Will be set based on user's preference
  ...
  Future<void> _loadUserPreference() async {
    try {
      final doc = await FirebaseFirestore.instance.collection('musicians').doc(widget.uid).get();
      if (doc.exists && mounted) {
        final method = doc.data()?['twoFactorMethod'] ?? 'sms';
        debugPrint('2FA Method from Firestore: $method');
        setState(() {
          _useSms = method == 'sms';
        });
        ...
        debugPrint('No document found for user');
        _sendEmailOtp(); // Default to email
      }
    } catch (e) {
      debugPrint('Error loading user preference: $e');
      _sendEmailOtp(); // Default to email
    }
  }
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Method Selection Card (Email or SMS)
                      Container(
