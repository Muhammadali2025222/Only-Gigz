...
class DmcaPolicyScreen extends StatefulWidget {
  const DmcaPolicyScreen({super.key});

  @override
  State<DmcaPolicyScreen> createState() => _DmcaPolicyScreenState();
}

class _DmcaPolicyScreenState extends State<DmcaPolicyScreen> {
  String _rawContent = '';
  ...
  void initState() {
    super.initState();
    _loadDmcaPolicyContent();
  }

  Future<void> _loadDmcaPolicyContent() async {
    // 1. Dynamic fetch from Cloud Firestore
    try {
      final snap = await FirebaseFirestore.instance.collection('policies').doc('dmca_policy').get();
      ...
      debugPrint('Firestore dmca_policy fetch warning: $e');
    }

    // 2. Local Asset Fallback
    try {
      final loaded = await rootBundle.loadString('assets/policies/dmca_policy.txt');
      if (mounted) {
        setState(() {
          _rawContent = loaded;
          _isLoading = false;
            ...
            children: [
              ...
              Padding(
                ...
                child: _isLoading
                    ...
                    : _buildParsedDmcaBody(_rawContent),
              ),
  ...
  Widget _buildParsedDmcaBody(String text) {
    ...
    for (var block in blocks) {
      ...
      // Handle Table Blocks (Section 2 Designated Copyright Agent Table)
      ...
      // Section Header detection (e.g. "1. Purpose and Scope", "2. Designated Copyright Agent", etc.)
