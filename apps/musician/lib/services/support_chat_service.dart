...
class SupportChatService extends ChangeNotifier {
  ...
  Future<void> sendMessage(String text, {required String userType, String? userName, bool isFeatured = false}) async {
    ...
    // If userName not provided, try to fetch from user profile
    String finalUserName = userName ?? 'User';
    if (userName == null || userName.isEmpty) {
      try {
        final userDoc = await _firestore.collection('musicians').doc(uid).get();
        if (userDoc.exists) {
          finalUserName = userDoc.data()?['fullName'] ?? 'Musician';
        }
      } catch (e) {
        debugPrint('Error fetching user name: $e');
        finalUserName = 'Musician';
      }
    }
