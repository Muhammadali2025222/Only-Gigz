...
class _PrivacyPolicyScreenState extends State<PrivacyPolicyScreen> {
  ...
  void initState() {
    super.initState();
    _loadPrivacyPolicyContent();
  }

  Future<void> _loadPrivacyPolicyContent() async {
    // 1. Dynamic fetch from Cloud Firestore
    try {
      final snap = await FirebaseFirestore.instance.collection('policies').doc('privacy_policy').get();
      ...
      debugPrint('Firestore privacy_policy fetch warning: $e');
    }

    // 2. Local Asset Fallback
    try {
      final loaded = await rootBundle.loadString('assets/policies/privacy_policy.txt');
      if (mounted) {
        setState(() {
          _rawContent = loaded;
          _isLoading = false;
            ...
            children: [
              ...
              Padding(
                ...
                child: _isLoading
                    ...
                    : _buildParsedPrivacyBody(_rawContent),
              ),
  ...
  Widget _buildParsedPrivacyBody(String text) {
