...
class AuthService extends ChangeNotifier {
      ...
      if (response.statusCode == 200) {
        ...
        try {
          ...
          if (token != null && _auth.currentUser != null) {
            ...
            await api.registerFcmToken(_auth.currentUser!.uid, 'organizer', token);
          }
        } catch (_) {}
  ...
  Future<String?> sendEmailOtp(String email, String uid) async {
    try {
      final actionCodeSettings = ActionCodeSettings(
        url: 'https://onlygigz-33557.firebaseapp.com/?app=organizer',
        handleCodeInApp: true,
        androidPackageName: 'com.onlygigz.organizer',
  ...
  Future<String?> updateProfile({
    ...
    try {
      final response = await http.post(
        Uri.parse('$_backendUrl/auth/profile/update'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'uid': uid,
          'name': name,
  ...
  Future<String?> updateOrganization({
    ...
    try {
      ...
      return _handleError(response, 'Failed to update organization');
    } catch (e) {
      return e.toString();
    }
  }
  ...
  Future<String> getUserStatus([String? uid]) async {
    ...
    try {
      final doc = await FirebaseFirestore.instance.collection('organizers').doc(targetUid).get();
      ...
      debugPrint('Error fetching organizer user status: $e');
      return 'approved';
    }
  }
}
