...
export function UserProfileModal({ isOpen, onClose, userData }: UserProfileModalProps) {
  ...
  const displayName = userData.fullName || userData.name || "Unknown User";
  const userRole = userData.role || (userData.fullName ? "musician" : "organizer");
          ...
          <div className="flex items-start gap-[24px]">
            ...
            <div className="w-[80px] h-[80px] rounded-full overflow-hidden bg-[#2A2A2A] flex items-center justify-center border border-white/10 shadow-lg">
              {userData.profileImageUrl ? (
                <img src={userData.profileImageUrl} alt={displayName} className="w-full h-full object-cover" />
              ) : (
            ...
            {/* Name and Status */}
            <div className="pt-2">
              <h3 className="text-white text-[24px] font-semibold leading-[32px] mb-2">{displayName}</h3>
              <div className="flex gap-2">
                <span className={`inline-flex items-center px-3 py-1 rounded-full text-[12px] font-bold leading-[16px] ${
                  userData.status === 'active' ? 'bg-[#10B981]/10 text-[#10B981]' : 
                  userData.status === 'pending' || userData.status === 'inactive' ? 'bg-[#ef4444]/10 text-[#ef4444]' : 
                  'bg-[#ef4444]/10 text-[#ef4444]'
                }`}>
                  {userData.status || "active"}
                </span>
                <span className="inline-flex items-center px-3 py-1 rounded-full bg-blue-500/10 text-blue-400 text-[12px] font-medium leading-[16px] capitalize">
                  {userRole}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-[12px]">
                ...
                <div>
                  ...
                  <p className="text-white text-[14px] font-medium leading-[20px]">{userData.contact || userData.businessPhone || userData.phone || "N/A"}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-[12px]">
                ...
                <div>
                  ...
                  <p className="text-white text-[14px] font-medium leading-[20px]">{userData.location || "N/A"}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-[12px]">
                ...
                <div>
                  ...
                  <p className="text-white text-[14px] font-medium leading-[20px]">{formatDate(userData.joinedAt || userData.createdAt)}</p>
                </div>
              </div>

              <div className="pt-2">
                <p className="text-[#999999] text-[12px] leading-[16px] mb-1">
                  {userRole === "musician" ? "Total Bookings" : "Total Gigs"}
                </p>
                <p className="text-[#A2F301] text-[24px] font-bold leading-[32px]">
                  {userData.bookings || userData.totalGigs || "0"}
                </p>
              </div>
            </div>
          </div>
